...
## Update awareness (Cookie fleet — disabled)

**Do not** run `scripts/check-update.mjs`. Cookie fleet sets `ARCHIFY_UPDATE_CHECK_DISABLED=1` on all hosts; agents must assume update checks are off and must not spawn network check-update.

Forkkeep / manual sync owns updates. Never phone home to GitHub Pages for skill manifests.
